...
/*
void PrintTree(HuffTree HT,int i,char (*tree)[100],int layer,char *cchar) //按树状打印二叉树 
{ 

    if(HT[i].lchild==0)
    {
        tree[layer][position]=cchar[i];
    }
    else
    {
        PrintTree(HT,HT[i].lchild,tree,layer+1,cchar);
        tree[layer][position]='*';
        PrintTree(HT,HT[i].rchild,tree,layer+1,cchar);
    }
    position++;
}
*/
